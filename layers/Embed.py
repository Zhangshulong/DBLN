...
class PositionalEmbedding(nn.Module):
    def __init__(self, d_model, max_len=20000):
        ...
        # div_term may be longer than the number of odd indices when d_model is odd.
        # Ensure we only use the needed length for the assignment to avoid size mismatch.
...
class PatchEmbedding(nn.Module):
    ...
    def forward(self, x):
        ...
        # Input encoding
        # print("PatchEmbedding x shape before embedding:", x.shape)
        # print("value_embedding x shape:", self.value_embedding(x).shape)
        # print("position_embedding x shape:", self.position_embedding(x).shape)
        x = self.value_embedding(x) + self.position_embedding(x)
        return self.dropout(x), n_vars
    
class DBLNEmbed(nn.Module):
    def __init__(self, decomp=True, seq_len=256, in_chans=3, patch_size=16,  embed_dim=256, dropout=0.1, ls_scale=2):
        super().__init__()
        patch_size_long = patch_size   # 长时序分量的 patch 大小
        stride_long = patch_size_long // 2  # 重复采样时，stride 为 patch_size 的一半
        num_patches_long = int((seq_len - patch_size_long) / stride_long + 1)
        self.num_patches_long = num_patches_long
        self.proj_long = nn.Conv1d(in_chans, embed_dim, kernel_size=patch_size_long, stride=stride_long, groups=1)

        self.decomp = decomp
        if self.decomp:
            patch_size_short = patch_size // ls_scale # 短时序分量的 patch 大小
            stride_short = patch_size_short // 2  # 重复采样时，stride 为 patch_size 的一半
            num_patches_short = int((seq_len - patch_size_short) / stride_short + 1)
            self.num_patches_short = num_patches_short
            self.proj_short = nn.Conv1d(in_chans, embed_dim, kernel_size=patch_size_short, stride=stride_short, groups=1)
                
    def forward(self, x): # x: [B, C, L]
        x_long = self.proj_long(x).flatten(2).transpose(1, 2)
        if self.decomp:
            x_short = self.proj_short(x).flatten(2).transpose(1, 2)
            return x_long, x_short
        else:
            return x_long


class TMEmbed(nn.Module):
    def __init__(self, seq_len=256, in_chans=3, patch_size=16,  embed_dim=256, dropout=0.1, ls_scale=2):
        super().__init__()
