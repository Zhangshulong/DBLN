...
class PatchEmbedding(nn.Module):
    def __init__(self, d_model, patch_len, stride, padding, dropout):
        ...
        self.stride = stride
        print(self.patch_len,self.stride)
    ...
    def __init__(self, args, pretrain=False):
        ...
        for i in range(self.num_task):
            ...
            if dataset_name not in self.prompt_tokens:
                self.prompt_tokens[dataset_name] = torch.zeros(1, args.enc_in, args.prompt_num, args.d_model)
                torch.nn.init.normal_(self.prompt_tokens[dataset_name], std=.02)
                self.mask_tokens[dataset_name] = torch.zeros(1, args.enc_in, 1, args.d_model)

            if args.task_name == 'classification':
                self.category_tokens[task_data_name] = torch.zeros(1, args.enc_in, args.num_class, args.d_model)
                torch.nn.init.normal_(self.category_tokens[task_data_name], std=.02)
                self.cls_tokens[task_data_name] = torch.zeros(1, args.enc_in, 1, args.d_model)
        ...
        self.cls_nums = {}
        for i in range(self.num_task):
            task_data_name = args.dataset
            if args.task_name == 'classification':
                self.cls_nums[task_data_name] = args.num_class
                ...
                input_pad = args.stride * (input_token_len - 1) + args.patch_len - args.seq_len
                pred_token_len = calculate_unfold_output_length(
                    args.pred_len-input_pad, args.stride, args.patch_len)
                real_len = args.seq_len + args.pred_len
                self.cls_nums[task_data_name] = [pred_token_len,args.pred_len, real_len]
