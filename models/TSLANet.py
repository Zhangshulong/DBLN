...
class Adaptive_Spectral_Block(nn.Module):
    ...
    def forward(self, x_in):
        ...
        real = self.complex_weight[:, 0].to(x.device)
        imag = self.complex_weight[:, 1].to(x.device)
        weight = torch.complex(real, imag)  # 形状 [dim], dtype=torch.complex64
        #weight = torch.view_as_complex(self.complex_weight.contiguous()).to(x.device)
        ...
        if self.adaptive_filter:
            ...
            #weight_high = torch.view_as_complex(self.complex_weight_high.contiguous()).to(x.device)
            real = self.complex_weight_high[:, 0].to(x.device)
            imag = self.complex_weight_high[:, 1].to(x.device)
            weight_high = torch.complex(real, imag)  # 形状 [dim], dtype=torch.complex64
...
class Model(nn.Module):
    ...
    def forward(self, x, x_mark, seg_mask, attn_mask):
        x = x.permute(0,2,1)  # B L C -> B C L
        
        x = self.patch_embed(x)
        x = x + self.pos_embed
        x = self.pos_drop(x)
        #print("x after pos embed shape:", x.shape)
